...
class SWCS_Importer {
    ...
    private static function upsert_product( $row, $variation_rows ) {
        ...
        if ( $is_variable ) {
            ...
            self::sync_variations( $product_id, $variation_rows );
        } else {
        ...
        return array( 'product_id' => $product_id, 'reference' => $reference, 'type' => $is_variable ? 'variable' : 'simple', 'variations' => count( $variation_rows ) );
    }
    ...
    private static function sync_variations( $product_id, $rows ) {
        ...
        foreach ( get_children( array( 'post_parent' => $product_id, 'post_type' => 'product_variation', 'post_status' => 'any', 'fields' => 'ids' ) ) as $id ) {
            ...
            if ( $variation ) { $existing[ $variation->get_sku() ] = $id; }
        }
        foreach ( $rows as $row ) {
            $mapped = SWCS_Mapper::variation( $row );
            $sku = $mapped['sku'];
            if ( '' === $sku ) { continue; }
            $variation = isset( $existing[ $sku ] ) ? new WC_Product_Variation( $existing[ $sku ] ) : new WC_Product_Variation();
